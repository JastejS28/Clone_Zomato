...
export const postRestaurant = ErrorWrapper(async (req, res, next) => {
    ...
    const email = req.user.email;        // loged in user ki email vo uthalega so do not need to write, also if written any different it will not be included
    console.log(email);
    if (!email) {
    ...
    try {
        ...
        throw new ErrorHandler(500,error.message || "Not able to add restaurant right now!");
    }
})
